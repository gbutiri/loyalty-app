...
function postAjax(d) {
	...
	for (el in d.localstore) {
        window.localStorage.setItem(el, d.localstore[el]);
		//el_split(el).html(d.localstore[el]);
	}
	for (el in d.htmls) {
...
$(document).on('doAjaxController', function (e, $this) {
	...
	if (_do_ajax) {
        ...
        if (_module.toLowerCase().indexOf('http') !== 0) {
            _module = '/' + _module;
        }
...
(function ($) {
	...
	$.fn.vbox = function (action, content) {
		...
		case "open":
			...
			$('body').css({
				'overflow' : 'hidden'
			}).append('<div class="fuzz" id="vbox_' + g_vboxlevel + '"><table align="center" class="vbox-table"><tr><td class="vbox-cell"><div class="vbox container-fluid"><div class="vbox-content">' + content + '</div><a href="#" class="vbox-close">&times;</a></div></td></tr></table></div>');
